...
const ExternalIcon = () => (
  <svg width="12" height="12" viewBox="0 0 12 12" fill="none" aria-hidden="true">
    <path d="M1.5 10.5L10.5 1.5M10.5 1.5H5M10.5 1.5V7" stroke="currentColor" strokeWidth="1.5"
...
// Cinematic gradient thumbnails per project
const thumbnailStyles: Record<string, React.CSSProperties> = {
  portfolio: {
    background: 'linear-gradient(135deg, #050c1a 0%, #071425 40%, #0a1f35 100%)',
  },
  'case-study': {
    background: 'linear-gradient(135deg, #060c1c 0%, #080f22 40%, #0d1530 100%)',
  },
  'task-manager': {
    background: 'linear-gradient(135deg, #050d1c 0%, #07111e 40%, #091828 100%)',
  },
}

// Abstract SVG decoration per project
...
const categoryStyle = (cat: string): React.CSSProperties => {
  ...
  return styles[cat] ?? { background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.10)', color: 'var(--text-secondary)' }
}
...
export function Projects() {
  ...
  return (
    <section id="projects" className="section-wrapper relative overflow-hidden" aria-label="Projects">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects.map((project, idx) => (
            <motion.article
              ...
              transition={{ duration: 0.45, delay: idx * 0.10, ease: 'easeOut' as const }}
                ...
                <div style={{ padding: '1.25rem', display: 'flex', flexDirection: 'column', flex: 1, gap: '0.875rem' }}>
                  ...
                  {(project.githubUrl ?? project.liveUrl) && (
                    <div style={{
                      ...
                      {project.liveUrl && (
                        <Button variant="primary" size="sm" rightIcon={<ExternalIcon />}
